...
(function (win, doc) {
  ...
  const loadCustomScript = function (mascot, config) {
  ...
  const chateleon = function (...args) {
    let config = {
      ...
      meta: {
        position: args[0].meta && args[0].meta.position || {
          type: "relative",
          right: "0%",
          bottom: "0%",
        },
        dimensions: args[0].meta && args[0].meta.dimensions || {
          height: "150px",
          width: "260px",
          zIndex: 500
        }
    ...
    function createAndAppendElementInDOM(mascot, containerId) {
      ...
      mascot.style.height = config.meta.dimensions.height || 150 + "px";
      mascot.style.width = config.meta.dimensions.width || 260 + "px";
      mascot.onclick = trackEvent;
      mascot.onmouseover = trackEvent;
      chateleonContainer.id = containerId;
      mascot.style.position = config.meta.position.type || "relative";
      mascot.style.right = config.meta.position.right || 0 + "px";
      mascot.style.bottom = config.meta.position.bottom || 0 + "px";
      ...
      chateleonContainer.style.height = mascot.style.height || 150 + "px";
      chateleonContainer.style.width = mascot.style.width || 260 + "px";
      chateleonContainer.style.zIndex = config.meta.dimensions.zIndex || config.dimensions.zIndex || 500;
      doc.body.appendChild(chateleonContainer);

      const isAppending = !!config.append;
      switch (isAppending) {
        case true:
          const elementHasId = config.append.id !== undefined;
    ...
    function appendAnimation(elementHasId, mascot, chateleonContainer) {
      ...
      switch (elementHasId) {
        case true:
          element = document.getElementById(config.append.id);
          if (element) {
            element.appendChild(chateleonContainer);
          }
          break;
        case false:
          const elementHasClass = config.append.class !== undefined;
          if (elementHasClass) {
            element = document.querySelector("." + config.append.class);
            if (element) {
              element.appendChild(chateleonContainer);
            }
          }
